...
subject=[]
result=["deces",'reanimation','hospitalises',]
...
params={#'regressorchain__base_estimator__tol':[i*0.001 for i in range(1,20)],
    'regressorchain__base_estimator__epsilon': [0.001],
    'regressorchain__base_estimator__C': [i*9000 for i in range (1,31)],
...
grid=GridSearchCV(model,param_grid=params,cv=2,scoring=scorer)
...
plt.plot(X_result,grid.predict(X_result)[:,:2])
plt.show()
#######partial_fit
